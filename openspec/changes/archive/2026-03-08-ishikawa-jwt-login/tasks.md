...
- [x] 1.1 In `/index.html`, replace `API_BASE` with `https://n8n.joinp.com/webhook` and remove the commented-out alternative URLs

## 2. Add login screen HTML and CSS

- [x] 2.1 In `/index.html`, add a login screen `<div id="loginScreen">` with email input, password input, submit button, and an error message element
...
- [x] 3.1 In `/index.html`, add a `checkAuth()` function: on page load, if `sessionStorage` has a token show the main app, otherwise show the login screen
...
- [x] 3.5 Add a logout button to the header in `/index.html` and wire it to `logout()`
- [x] 3.6 Call `checkAuth()` on page load

## 4. Attach JWT to authenticated requests

- [x] 4.1 In the XHR upload handler in `/index.html`, add `xhr.setRequestHeader('Authorization', 'Bearer ' + sessionStorage.getItem('token'))` before `xhr.send()`
- [x] 4.2 In the `fetch` chat handler in `/index.html`, add `Authorization: 'Bearer ' + sessionStorage.getItem('token')` to the request headers
