...
`/index.html` is a single-file vanilla JS app with no build step. It currently exposes the full RAG interface (file upload + chat) to anyone with the URL. The backend at `https://n8n.joinp.com/webhook` already implements JWT auth — the frontend just needs to participate.
